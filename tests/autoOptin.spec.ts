...
test.describe('Auto Opt-In Config - Create Popup Tests', () => {
    ...
    test('TC-29 Verify Save stays disabled until Sport Wager\'s mandatory fields (Minimum Required Amount, Odds) are filled — Number Of Legs accepts values >= 0 and does not block Save', async ({ page, autoOptinPage }, testInfo) => {
        const name = `AutoOptIn_${Date.now()}`;
        ...
        // Business rule change: Number Of Legs now accepts any value >= 0 (including being left
        // empty, which the app treats as 0) — it no longer blocks Save the way Minimum Required
        // Amount and Odds still do.
        await autoOptinPage.fillOdds('1.50');
        await page.waitForTimeout(200);
        await expect(autoOptinPage.saveBtn, 'Expected Save to become enabled once Minimum Required Amount and Odds are filled, even with Number Of Legs left empty').toBeEnabled({ timeout: 10000 });

        await autoOptinPage.fillNumberOfLegs('0');
        await page.waitForTimeout(200);
        await expect(autoOptinPage.saveBtn, 'Expected Save to stay enabled with Number Of Legs set to 0').toBeEnabled();

        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC-29_sport_wager_mandatory_fields');
        console.log('Save becomes enabled once Minimum Required Amount and Odds are filled; Number Of Legs accepts values >= 0 (including 0 and empty) without blocking Save.');
...
// tor('.p-toast-message-text').first();
