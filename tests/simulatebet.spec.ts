...
// Clicks the Bets table's "+" (Add) button and removes the dangling blank "next entry" row
// it also appends. Confirmed live (captured outerHTML): clicking + doesn't just commit the
// filled row — it appends a fresh, blank row right after it (own Feed dropdown still on
// placeholder). Simulate requires EVERY row in the table to be valid, not just the one just
// filled in, so that dangling blank row alone keeps Simulate disabled even once the actual bet
// is complete. It's the newest row, so it's last in document order — remove it, leaving only
// the valid bet.
async function addBetAndRemoveDanglingRow(page: Page, betsTable: Locator): Promise<void> {
  const addBtn = betsTable.locator('button:has(.pi-plus)').first();
  await addBtn.click();
  await page.waitForTimeout(300);

  const deleteButtons = betsTable.locator('button:has(.pi-trash)');
  await expect(deleteButtons.first(), 'Expected a bet row to appear in the table after clicking Add').toBeVisible({ timeout: 5000 });
...
test.describe('Simulate Bet - Settlements - Bets Table and Simulation', () => {
  ...
  //   await selectFirstDropdownOption(page, dropdownByAriaLabel(betsTable, 'Sport *'));
  //   const leagueDropdown = dropdownByAriaLabel(betsTable, 'League');
  //   const leagueSelection = await selectFirstDropdownOption(page, leagueDropdown);
  //   // Confirm the selection actually registered before clicking Add — a race between the
  //   // dropdown's model update and the Add button's click was a plausible cause of Add silently
  //   // treating this row as Feed+Sport-only (the same "invalid bet" shape TC-16 covers).
  //   await expect(leagueDropdown.locator('.p-dropdown-label'), 'Expected League to show the selected value before adding the bet').toHaveText(leagueSelection, { timeout: 5000 });

  //   const addBtn = betsTable.locator('button:has(.pi-plus)').first();
  //   await addBtn.click();
  //   await page.waitForTimeout(300);

  //   // Confirm the bet row actually got added (a delete/trash button appears per row) before
  //   // blaming the rest of the form — distinguishes "Add didn't add anything" from "it was
  //   // added but the app doesn't consider League alone sufficient".
  //   const deleteButtons = betsTable.locator('button:has(.pi-trash)');
  //   await expect(deleteButtons.first(), 'Expected a bet row to appear in the table after clicking Add').toBeVisible({ timeout: 5000 });

  //   // Confirmed live (captured outerHTML): clicking + doesn't just commit the filled row — it
  //   // also appends a fresh, blank "next entry" row right after it (own Feed dropdown still on
  //   // placeholder). Simulate requires EVERY row in the table to be valid, not just one, so that
  //   // dangling blank row alone keeps it disabled even though the actual bet is complete. It's
  //   // the newest row, so it's last in document order — remove it, leaving only the valid bet.
  //   if (await deleteButtons.count() > 1) {
  //     await deleteButtons.last().click();
  //     await page.waitForTimeout(300);
  //   }
  ...
  test('TC-18 Verify the Delete button removes a bet from the Bets table', async ({ page }, testInfo) => {
    ...
    await addBetAndRemoveDanglingRow(page, betsTable);
...
test.describe('Simulate Bet - Bets Placed - Bets Table and Simulation', () => {
  ...
  //   // Confirm the selection actually registered before clicking Add — a race between the
  //   // dropdown's model update and the Add button's click was a plausible cause of Add silently
  //   // treating this row as Feed+Sport-only (the same "invalid bet" shape TC-41 covers).
  //   await expect(leagueDropdown.locator('.p-dropdown-label'), 'Expected League to show the selected value before adding the bet').toHaveText(leagueSelection, { timeout: 5000 });
  ...
  //   // dangling blank row alone keeps it disabled even though the actual bet is complete. It's
  //   // the newest row, so it's last in document order — remove it, leaving only the valid bet
  //   // (the one this test's own Delete assertion below still needs to remain).
  //   if (await deleteButtons.count() > 1) {
  //     await deleteButtons.last().click();
  //     await page.waitForTimeout(300);
  //   }
