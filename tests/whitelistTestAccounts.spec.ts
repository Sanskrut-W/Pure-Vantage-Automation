...
test.describe('Whitelist Test Accounts Tests', () => {
    ...
    test('TC_03 Verify creation of Whitelist Test Account', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        ...
        await whitelistTestAccountsPage.fillForm({
            username:  unique,
            accountId: CommonUtils.generateUUID(),
            mobile:    '27700000001',
    ...
    test('TC_07 Verify Account ID field character limit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        ...
        // Either maxlength restricted input, validation error shown, or Save disabled
        const wasRestricted = actualValue.length < longValue.length;
        const errorShown    = await whitelistTestAccountsPage.isValidationErrorVisible();
        const saveDisabled  = await whitelistTestAccountsPage.isSaveButtonDisabled();
        expect(wasRestricted || errorShown || saveDisabled, 'Character limit must be enforced on Account ID').toBe(true);
    ...
    test('TC_10 Verify Username field character limit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        ...
        const errorShown    = await whitelistTestAccountsPage.isValidationErrorVisible();
        const saveDisabled  = await whitelistTestAccountsPage.isSaveButtonDisabled();
        expect(wasRestricted || errorShown || saveDisabled, 'Character limit must be enforced on Username').toBe(true);
    ...
    test('TC_13 Verify Mobile Number field accepts short values (no minimum length enforced)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const unique = `TAF-WL-G-${Date.now()}`;
        await whitelistTestAccountsPage.openCreateDialog();

        // Confirmed: this app does not enforce a minimum length on Mobile Number
        await whitelistTestAccountsPage.fillForm({
            username:  unique,
            accountId: CommonUtils.generateUUID(),
            mobile:    '12',
        ...
        await expect(page.locator('div.p-dialog').first()).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_13_mobile_short_accepted');
    });
    ...
    test('TC_16 Verify Email Address field character limit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        ...
        const errorShown    = await whitelistTestAccountsPage.isValidationErrorVisible();
        const saveDisabled  = await whitelistTestAccountsPage.isSaveButtonDisabled();
        expect(wasRestricted || errorShown || saveDisabled, 'Character limit must be enforced on Email Address').toBe(true);
    ...
    test('TC_18 Verify duplicate Username is allowed (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        await whitelistTestAccountsPage.openCreateDialog();
        ...
        // Confirmed: Username is not unique-constrained — save must succeed
        await expect(page.locator('div.p-dialog').first()).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_18_duplicate_username_allowed');
    });

    test('TC_19 Verify duplicate Mobile Number is allowed (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        await whitelistTestAccountsPage.openCreateDialog();
        ...
        // Confirmed: Mobile Number is not unique-constrained — save must succeed
        await expect(page.locator('div.p-dialog').first()).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_19_duplicate_mobile_allowed');
    });

    test('TC_20 Verify duplicate Email Address is allowed (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        // Create an account first to get a known email, then reuse it
        ...
        // Now create another account with the same email
        await whitelistTestAccountsPage.openCreateDialog();
        await whitelistTestAccountsPage.fillForm({
            username:  `${unique}-dup`,
            accountId: CommonUtils.generateUUID(),
        ...
        // Confirmed: Email Address is not unique-constrained — save must succeed
        await expect(page.locator('div.p-dialog').first()).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_20_duplicate_email_allowed');
    });
    ...
    test('TC_23 Verify Account ID field can be edited', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        // Edit only an account we ourselves created — never touch real/existing data
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editid');
        await whitelistTestAccountsPage.openEditFor(username);

        const dialog = page.locator('div.p-dialog').first();
        await expect(dialog).toBeVisible({ timeout: 10000 });

        const updatedId = CommonUtils.generateUUID();
    ...
    test('TC_24 Verify Username field can be edited', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-edituser');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_25 Verify Mobile Number field can be edited', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editmobile');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_26 Verify Email Address field can be edited', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editemail');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_27 Verify Region dropdown can be updated', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editregion');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_28 Verify mandatory field validation in Edit pop-up', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editmandatory');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_29 Verify invalid Mobile Number validation during edit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editmobileinvalid');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_30 Verify invalid Email Address validation during edit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editemailinvalid');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_31 Verify duplicate Account ID validation during edit', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        // Edit our own test account and attempt to set its Account ID to a known-existing one
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editdupacct');
        await whitelistTestAccountsPage.openEditFor(username);
    ...
    test('TC_32 Verify duplicate Username is allowed during edit (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editdupuser');
        await whitelistTestAccountsPage.openEditFor(username);
        ...
        // Confirmed: Username is not unique-constrained — save must succeed
        await expect(dialog).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_32_edit_dup_username_allowed');
    });

    test('TC_33 Verify duplicate Mobile Number is allowed during edit (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        const { username } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-editdupmobile');
        await whitelistTestAccountsPage.openEditFor(username);
        ...
        // Confirmed: Mobile Number is not unique-constrained — save must succeed
        await expect(dialog).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_33_edit_dup_mobile_allowed');
    });

    test('TC_34 Verify duplicate Email Address is allowed during edit (only Account ID is unique)', async ({ page, whitelistTestAccountsPage }, testInfo) => {
        // Use email created fresh so it's a guaranteed duplicate
        const unique = `TAF-WL-dup-em2-${Date.now()}`;
        const knownEmail = `${unique}@test.com`;

        // Create a reference account holding the email we'll try to duplicate
        await whitelistTestAccountsPage.createTestAccount('TAF-WL-dupemailref', {
            username: unique,
            mobile:   '27700000013',
            email:    knownEmail,
        });

        // Create a second, separate account of our own — this is the one we'll edit
        const { username: editTargetUsername } = await whitelistTestAccountsPage.createTestAccount('TAF-WL-dupemailtarget');
        await whitelistTestAccountsPage.openEditFor(editTargetUsername);
        ...
        // Confirmed: Email Address is not unique-constrained — save must succeed
        await expect(dialog).not.toBeVisible({ timeout: 10000 });
        await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_34_edit_dup_email_allowed');
    });
