...
test.describe('Marketing - Generic Wheel', () => {
  ...
  test('TC_35 - Verify numeric fields boundary values', async ({ page }, testInfo) => {
    ...
    // ── Scenario C: Enter very large values ───────────────────────────────────
    // Number of Slices max = 8; 9999 is clearly above it, keeping Save disabled.
    await setField(dialog.locator('#spinsPerUserInput input'), '99999');
    await setField(dialog.locator('#dailySpinLimitInput input'), '99999');
    await setField(dialog.locator('#spinValidityDaysInput input'), '99999');
    await setField(dialog.locator('#numberOfSlicesInput input'), '9999');
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_35-GenericWheel-BoundaryLarge');
    // Save must remain disabled — Number of Slices is above max
    await expect(saveBtn35).toBeDisabled({ timeout: 3000 });
  ...
  // TC_75: Verify numeric field boundaries — 0, negative, and very large values
  test('TC_75 - Verify numeric field boundary values are handled correctly on Edit', async ({ page }, testInfo) => {
    ...
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_75-GenericWheel-BoundaryZero');

    // Scenario B: enter very large values
    await setNumericField(dialog75.locator('#spinsPerUserInput input'), '99999');
    await setNumericField(dialog75.locator('#dailySpinLimitInput input'), '99999');
    await setNumericField(dialog75.locator('#spinValidityDaysInput input'), '99999');
    await setNumericField(dialog75.locator('#numberOfSlicesInput input'), '99');
    await expect(saveBtn75).toBeDisabled({ timeout: 3000 });

    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_75-GenericWheel-BoundaryLarge_success');
  });
  ...
  test('TC_94 - Verify Deactivate promotion - Click Yes on confirmation', async ({ page }, testInfo) => {
    const SLICES = 4; // must match numberOfSlices passed to createPromoForTest
    const promoName94 = await createPromoForTest(page, 'DeactYes_TC94', 5);
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_94-GenericWheel-PromoCreated');

    // Find the created promotion (include inactive toggle ON + search by name)
    const container94 = await findCreatedPromo(page, promoName94);

    // ── Step 1: Navigate to View Promotion Prizes ────────────────────────────
    await container94.locator('tbody tr[data-pc-section="bodyrow"]').first().locator('button.pure__table-menu-trigger').click();
    const viewPrizesItem94 = page.locator('a.p-menuitem-link[aria-label="View promotion prizes"]').first();
    await viewPrizesItem94.waitFor({ state: 'visible', timeout: 5000 });
    await viewPrizesItem94.click();
    const prizeContainer94 = page.locator('generic-wheel-promotion-prizes');
    await prizeContainer94.waitFor({ state: 'visible', timeout: 20000 });
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_94-GenericWheel-PrizePage');

    // ── Step 2: Create one prize per slice ──────────────────────────────────
    for (let i = 0; i < SLICES; i++) {
      await page.locator('button[aria-label="Create Prize"]').click();
      const prizeDialog94 = page.locator('div[role="dialog"]').first();
      await expect(prizeDialog94).toBeVisible({ timeout: 10000 });

      // Prize Type (mandatory dropdown)
      await prizeDialog94.locator('#prizeTypeDropdown [data-pc-section="trigger"]').click();
      const prizeTypePanel94 = page.locator('.p-dropdown-panel');
      await expect(prizeTypePanel94).toBeVisible({ timeout: 5000 });
      await prizeTypePanel94.locator('[data-pc-section="option"]:not(option), .p-dropdown-item, li[role="option"]').first().waitFor({ state: 'visible', timeout: 5000 });
      await prizeTypePanel94.locator('[data-pc-section="option"]:not(option), .p-dropdown-item, li[role="option"]').first().click();
      await page.waitForTimeout(200);

      // Display Text (mandatory)
      await prizeDialog94.locator('#displayTextInput').fill(`Prize_${i + 1}_TC94`);
      await page.waitForTimeout(200);

      // Save prize
      await prizeDialog94.locator('button[aria-label="Save"], button:has-text("Save")').first().click();
      await expect(prizeDialog94).toBeHidden({ timeout: 15000 });
      await page.waitForTimeout(300);
    }
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_94-GenericWheel-AllPrizesCreated');

    // ── Step 3: Click Back to return to Generic Wheel ───────────────────────
    const backBtn94 = prizeContainer94.locator('button.btn--info.p-component.p-button-icon-only');
    await backBtn94.waitFor({ state: 'visible', timeout: 5000 });
    await backBtn94.click();
    await page.locator('generic-wheel').waitFor({ state: 'visible', timeout: 20000 });
    await page.waitForTimeout(500);

    // ── Step 4: Re-enable Include Inactive toggle and find the promotion ─────
    const container94b = await findCreatedPromo(page, promoName94);

    // ── Step 5: Click three dots → Activate ─────────────────────────────────
    await container94b.locator('tbody tr[data-pc-section="bodyrow"]').first().locator('button.pure__table-menu-trigger').click();
    const activateItem94 = page.locator('a.p-menuitem-link[aria-label="Activate"]').first();
    await activateItem94.waitFor({ state: 'visible', timeout: 5000 });
    await activateItem94.click();
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_94-GenericWheel-ActivateConfirmDialog');

    // ── Step 6: Confirmation dialog — click Yes ──────────────────────────────
    ...
    // ── Step 7: Verify activation via success toast message ──────────────────
    const successToast94 = page.locator('.p-toast-message-success, .p-toast .p-toast-message, [data-p-severity="success"]').first();
    await expect(successToast94).toBeVisible({ timeout: 10000 });
    await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC_94-GenericWheel-ActivateYes_success');
  });
