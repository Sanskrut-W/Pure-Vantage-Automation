...
test.describe('Cashback Promotions - Edit Popup Tests', () => {
    ...
    test('TC-34 Verify Comp Percentage > 100 is rejected on Update', async ({ page, cashbackPage }, testInfo) => {
        ...
        // Move focus to another field (without changing it) to force the Comp Percentage
        // field's own blur/validation to run, then check whether it self-corrected.
        await cashbackPage.clickAnotherFieldInDialog(dialog);
        await page.waitForTimeout(300);

        const percentageAfterBlur = await cashbackPage.getCompPercentageValueInDialog(dialog);
        console.log(`Comp Percentage after entering 101 and moving focus away: "${percentageAfterBlur}"`);

        if (percentageAfterBlur === '100') {
            console.log('Comp Percentage > 100 correctly clamped down to 100 — rejected.');
            await CommonUtils.captureScreenshot(page, testInfo, 'reports/screenshots', 'TC-34_comp_pct_range');
            return;
        }

        // Field kept a value above 100 — fall back to checking whether Save itself blocks it.
        await cashbackPage.clickSaveInDialog(dialog);
        await page.waitForTimeout(2000);
        ...
        expect(dialogOpen || toastShown || fieldError, `Expected Comp Percentage to be clamped to 100 (was "${percentageAfterBlur}") or Save to be blocked`).toBe(true);
