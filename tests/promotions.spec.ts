...
test.describe("Promotion config test suite", () => {
    ...
    test.beforeEach("Navigate to Promotion config page from sidebar", async ({ page, sidebarPage, promotionConfigPage }) => {
        ...
        // This deep link doesn't always fully hydrate the Angular/Stencil
        // component on first load — reload once if nothing rendered yet.
        // Create Promotion is the most basic, always-present element on the
        // page, so its absence signals the whole page failed to hydrate.
        const loaded = await promotionConfigPage.createPromotionBtn.first()
            .waitFor({ state: 'visible', timeout: 15000 }).then(() => true).catch(() => false);
        if (!loaded) {
            await page.reload();
            await page.waitForLoadState('networkidle');
            await promotionConfigPage.createPromotionBtn.first().waitFor({ state: 'visible', timeout: 30000 });
        }
    })
    ...
    test("TC-13 Edit promotion details", async ({ promotionConfigPage }, testInfo) => {
        ...
        await promotionConfigPage.page.getByRole('button', { name: "Update Promotion Details" }).locator('..').getByRole('button').first().click();
        await promotionConfigPage.page.waitForLoadState('networkidle');
        // The renamed promotion may not be on page 1 of the list — filter by name first
        const listSearch = promotionConfigPage.page.locator('input.pure-input[placeholder="Search"]').first();
        await listSearch.waitFor({ state: 'visible', timeout: 10000 });
        await listSearch.fill(editedName);
        await promotionConfigPage.page.waitForTimeout(800);
