...
export class CompPayoutErrorsPage extends BasePage {
    ...
    constructor(page: Page) {
        super(page);
        this.searchInput = this.page.locator(compPayoutErrorsLocators.searchInput);
        this.table = this.page.locator('table.p-datatable-table, .p-datatable').first();
        this.compConfigCodeValue = this.page.locator(compPayoutErrorsLocators.compConfigCodeValue).first();
    }

    async searchByAccountId(accountId: string) {
        console.log(`Searching Comp Payout Error Details for Account Id: ${accountId}`);
    ...
    async clickViewOnRow(row: Locator) {
        console.log('Clicking View on Comp Payout Error row...');
        await this.clickElement(row.locator(compPayoutErrorsLocators.viewBtn));
    }
}
